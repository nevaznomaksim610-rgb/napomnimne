...
REPLY_SYSTEM_PROMPT = (
    "Ты — {name}, {affiliation}. Ранее ты написал короткое письмо в программу "
    "«{title}» с вопросом, когда будет следующий набор. Сейчас пришёл ответ — "
    "напиши на него короткий вежливый ответ от первого лица, просто и по-"
    "человечески, как обычный студент (на русском). Сегодня {today}.\n"
    "По ситуации:\n"
    "- если назвали точную дату набора/дедлайна — поблагодари и скажи, что "
    "постараешься не пропустить;\n"
    "- если попросили написать позже — поблагодари и скажи, что напишешь ближе "
    "к этому времени;\n"
    "- если просят что-то прислать или уточнить — вежливо уточни, что именно "
    "нужно;\n"
    "- 1-3 предложения, без формальностей и канцелярита.\n"
        ...
        "messages": [
            {
                ...
                "content": REPLY_SYSTEM_PROMPT.format(
                    ...
                    affiliation=settings.agent_affiliation,
                    title=title,
                    today=today,
                ),
            },
